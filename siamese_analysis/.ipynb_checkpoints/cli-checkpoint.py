...
def parse_args():
    ...
    parser.add_argument('--output-dir', type=str, default='siamese_results_v3_100000triplets_physical_4faultgroups_48samplespergroupBESTMODEL1', 
    ...
    parser.add_argument('--num-triplets', type=int, default=100000, 
                        help='Number of triplets to generate')
    parser.add_argument('--labels-to-use', nargs='+', 
                        help='Labels to use for training (default: all)')
    parser.add_argument('--include-physical', action='store_true', default=True,
...
def run_training(args):
    ...
    # Filter labels if specified (use 50% of rotations from each rotation speed from:
    # normal
    # imbalance_fault_6g or changed to use all imbalance faults
    # overhang_ball_fault_0g or changed to use all overhang ball faults
    # Group all normal rotation speeds as a single label
    samples_by_label = {}
    for label, samples in samples_by_label_eval.items():
        if label.startswith('normal'):
            ...
            samples_by_label['normal'].extend(samples[:1])
            """ if label not in samples_by_label:
                samples_by_label[label] = []
            samples_by_label[label].extend(samples[:10])  # Use at least 5 samples instead of 1 """
        elif label.startswith('imbalance_fault'):
            if label not in samples_by_label:
                samples_by_label[label] = []
            samples_by_label[label].extend(samples[:48])  # Use at least 5 samples instead of 1
        elif label.startswith('overhang_ball_fault'):
            if label not in samples_by_label:
                samples_by_label[label] = []
            samples_by_label[label].extend(samples[:48])  # Use at least 5 samples instead of 1
        elif label.startswith('horizontal_misalignment_fault'):
            if label not in samples_by_label:
                samples_by_label[label] = []
            samples_by_label[label].extend(samples[:48])  # Use at least 5 samples instead of 1
        elif label.startswith('vertical_misalignment_fault'):
            if label not in samples_by_label:
                samples_by_label[label] = []
            samples_by_label[label].extend(samples[:48])  # Use at least 5 samples instead of 1
    ...
    anchors, positives, negatives = generate_triplets_from_labeled_samples(
        samples_by_label, processor, args.num_triplets
    )
