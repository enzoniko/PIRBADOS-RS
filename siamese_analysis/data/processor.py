...
class ResidualProcessor:
    ...
    def __init__(self, fft_length=128, num_channels=4, advanced_features=False,
        ...
        if advanced_features:
            ...
            # Reduce workers for Windows compatibility
            if self.num_workers is None:
                import platform
                if platform.system() == 'Windows':
                    self.num_workers = 2  # Limit workers on Windows
    ...
    def fit_transform(self, samples: List[Union[Dict, np.ndarray]]) -> np.ndarray:
        ...
        if not self.advanced_features:
            ...
            print("Using advanced feature extraction from v1")
            ...
            try:
                ...
                processed_samples = []
                for sample in tqdm(sample_data, desc="Sequential processing"):
                    try:
                        # Slice sample to only use specified number of channels
                        sample = np.asarray(sample)
                        if sample.ndim == 1:
                            sample = sample.reshape(-1, 1)
                        if sample.shape[1] > self.num_channels:
                            sample = sample[:, :self.num_channels]

                        from .feature_extractors import extract_features_combined
                        features = extract_features_combined(
                            sample,
                            sample_rate=self.sampling_rate,
                            wavelet=self.wavelet,
                        ...
                        print(f"Failed to process sample: {sample_e}, using zeros")
                        # Create a zero array with reasonable shape
                        features = np.zeros((self.num_channels, 128))  # Use specified number of channels
                        processed_samples.append(features)

                processed = np.array(processed_samples)
...
def organize_residuals_by_label(residuals_dict: Dict[str, List[Dict]]) -> Dict[str, List[LabeledSample]]:
    """
    Organize residuals by data type labels (for PINN residuals to ensure sufficient samples per label)

    Args:
        residuals_dict: Dictionary mapping data types to lists of samples

    Returns:
        Dictionary mapping data type labels to lists of LabeledSample objects
    """
    labeled_samples = {}
    ...
    for data_type, samples in residuals_dict.items():
        for sample in samples:
            ...
            # Add to dict by level2_label (data type only) to ensure enough samples per label for triplet generation
            if labeled_sample.level2_label not in labeled_samples:
                labeled_samples[labeled_sample.level2_label] = []
            labeled_samples[labeled_sample.level2_label].append(labeled_sample)
    
    # Log statistics about the organization
    print(f"Organized samples into {len(labeled_samples)} data type labels:")
    ...
    if source == "direct":
        ...
        if has_data_residuals and has_physical_residuals:
            ...
            for key in residuals_dict:
                ...
                print(f"Processing {key}: {len(residuals_dict[key])} samples")

                # Check if data is already segmented (PINN residuals case)
                if isinstance(residuals_dict[key], list) and len(residuals_dict[key]) > 0:
                    first_item = residuals_dict[key][0]

                    # If it's already a list of dicts with 'data' and 'rot_speed', group by data_type only
                    if isinstance(first_item, dict) and 'data' in first_item and 'rot_speed' in first_item:
                        print(f"  {key} already segmented ({len(residuals_dict[key])} samples) - grouping by data type")
                        # Convert to list format expected by organize_residuals_by_label
                        processed_samples = []
                        for item in residuals_dict[key]:
                            processed_samples.append({
                                'data': item['data'],
                                'rot_speed': item['rot_speed']
                            })
                        residuals_dict[key] = processed_samples
                        continue

                # Original segmentation logic for raw data files
        ...
        if has_data_residuals and has_physical_residuals:
            ...
            for data_type, sequences in residuals_dict.items():
                ...
                print(f"Processing hybrid {data_type}: {len(sequences)} sequences")

                # Check if data is already segmented (hybrid PINN residuals case)
                if isinstance(sequences, list) and len(sequences) > 0:
                    first_item = sequences[0]

                    # If it's already a list of dicts with 'data' and 'rot_speed', convert to LabeledSample format
                    if isinstance(first_item, dict) and 'data' in first_item and 'rot_speed' in first_item:
                        print(f"  {data_type} already segmented ({len(sequences)} samples)")
                        for seq_data in sequences:
                            ...
                            # Use level2_label (data type only) for PINN residuals to ensure enough samples per label
                            if labeled_sample.level2_label not in processed:
                                processed[labeled_sample.level2_label] = []
                            processed[labeled_sample.level2_label].append(labeled_sample)
                        continue

                # Original hybrid processing
                processed_sequences = []
                for seq_idx, seq_data in sequences.items():
                    # Extract the data and rotation speed from the sequence
                    data = seq_data['data'].numpy() if hasattr(seq_data['data'], 'numpy') else seq_data['data']
                    ...
                    # Add to dict by level2_label (data type only) for PINN residuals to ensure enough samples per label
                    if labeled_sample.level2_label not in processed:
                        processed[labeled_sample.level2_label] = []
                    processed[labeled_sample.level2_label].append(labeled_sample)
