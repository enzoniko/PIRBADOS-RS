...
class PINN(nn.Module):
    ...
    def __init__(self):
        ...
        self.NNforUnmeasured = nn.Sequential(
            nn.Linear(14, 64), # 8 inputs (x2_dot, y2_dot, x3_dot, y3_dot, x2, y2, x3, y3, M1, D1, K1, K2, E1, Omega)
            nn.Tanh(),
            nn.Linear(64, 64),
            nn.Tanh(),
    ...
    def forward(self, x):
        ...
        M1 = torch.clamp(self.M1, 0.0)    
        M2 = torch.clamp(self.M2, 0.0)
        M3 = torch.clamp(self.M3, 0.0)
        D1 = torch.clamp(self.D1, 0.0)
        D2 = torch.clamp(self.D2, 0.0)
        D3 = torch.clamp(self.D3, 0.0)
        K1 = torch.clamp(self.K1, 1e-8)
        K2 = torch.clamp(self.K2, 0.0)
        E1 = torch.clamp(self.E1, 0.0)
    ...
    def compute_residuals(self, x, pred):
        ...
        residual1 = K1*x2 + K2*x3 - self.fA
        residual2 = K1*y2 + K2*y3 - M1*self.g - self.fB
        residual3 = M3*x3_ddot + D3*x3_dot + K2*x3 - (K2/K1)*x2_ddot - (K2/K1)*x2_dot - K2*x2 - self.fC
        residual4 = M3*y3_ddot + D3*y3_dot + K2*y3 - (K2/K1)*y2_ddot - (K2/K1)*y2_dot - K2*y2 - (K2/K1)*M2*self.g + M3*self.g - self.fD
