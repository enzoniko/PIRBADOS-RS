...
class PINN(nn.Module):
    ...
    def __init__(self):
        ...
        self.NNforUnmeasured = nn.Sequential(
            ...
            nn.Linear(64, 64),
            nn.Tanh(),
            nn.Linear(64, 2) # 2 outputs (fC, fD)
        )
    ...
    def forward(self, x):
        ...
        fC, fD = torch.split(self.NNforUnmeasured(input_for_unmeasured), 1, dim=1)
    ...
    def compute_residuals(self, x, pred):
        ...
        # Eq1 = fA # Not considered in josafat version
        # Eq2 = fB # Not considered in josafat version
        # Eq3 = fC
        # Eq4 = fD
...
def custom_loss(model, x, y_true, max_x_values, min_x_values, max_y_values, min_y_values):
    ...
    # Compute residuals
    residual3, residual4 = model.compute_residuals(x, y_pred)
    ...
    physics_loss = torch.sqrt(torch.mean(torch.stack([torch.mean(residual**2) for residual in [residual3, residual4]])))

    # Define the total loss as the sum of the data loss and the physics-informed loss
    print(data_loss.detach(), physics_loss.detach(), end='\r')
