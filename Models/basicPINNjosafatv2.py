...
class PINN(nn.Module):
    ...
    def __init__(self):
        ...
        # fA: [F_{x}(x_3, y_3, \dot{x_3}, \dot{y_3}) - \frac{K_2}{K_1}F_{x}(x_2, y_2, \dot{x_2}, \dot{y_2})]
        # fB: [F_{y}(x_3, y_3, \dot{x_3}, \dot{y_3}) - \frac{K_2}{K_1}F_{y}(x_2, y_2, \dot{x_2}, \dot{y_2})]

        self.NNforUnmeasured = nn.Sequential(
            nn.Linear(10, 64), # 8 inputs (x2_dot, y2_dot, x3_dot, y3_dot, x2, y2, x3, y3, K1, K2)
            nn.Tanh(),
            nn.Linear(64, 64),
            nn.Tanh(),
            nn.Linear(64, 2) # 2 outputs (fA, fB)
        )
        ...
        # Define learnable parameters (masses, damping coefficients, spring constants) 
        # Using only parameters comprised in physical equations, since these are the only learnable ones.
        ...
        # Define the Angular Velocity (omega) (according to Josafat it is a constant)
        # I think it is variant in time (in this case it could be added as input)
        # For now I will define it as learnable parameter
    ...
    def forward(self, x):
        ...
        fA, fB = torch.split(self.NNforUnmeasured(input_for_unmeasured), 1, dim=1)
    ...
    def compute_residuals(self, x, pred):
        ...
        residual3 = M3*x3_ddot + D3*x3_dot + K2*x3 - (K2/K1)*x2_ddot - (K2/K1)*x2_dot - K2*x2 - self.fA
        residual4 = M3*y3_ddot + D3*y3_dot + K2*y3 - (K2/K1)*y2_ddot - (K2/K1)*y2_dot - K2*y2 - (K2/K1)*M2*self.g + M3*self.g - self.fB

        return residual3, residual4

def custom_loss(model, x, y_true, max_x_values, min_x_values, max_y_values, min_y_values, alpha=1.0, beta=1.0):
    ...
    # Compute residuals
    residual1, residual2 = model.compute_residuals(x, y_pred)

    # Compute the physics-informed loss
    physics_loss = torch.sqrt(torch.mean(torch.stack([torch.mean(residual**2) for residual in [residual1, residual2]])))
