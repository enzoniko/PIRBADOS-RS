...
def custom_loss(model, x, y_true, max_x_values, min_x_values, max_y_values, min_y_values):
    ...
    #x_denorm = x * (max_x_values - min_x_values) + min_x_values
    ...
    resMass1_loss = residualMass1**2
    resMass2_loss = residualMass2**2
    
    # Define a gentler loss term to penalize negative values of the learnable parameters
    # This will be a sum of relu functions applied to the learnable parameters
    #gama = 10  # Reduced from 100 to avoid too harsh penalties
    """ neg_loss = (
        torch.relu(-model.M1) + torch.relu(-model.M2) + torch.relu(-model.M3) +
        torch.relu(-model.D1) + torch.relu(-model.D2) + torch.relu(-model.D3) +
        torch.relu(-model.K1) + torch.relu(-model.K2) + torch.relu(-model.E1)
    ) """
    
    # Scale the neg_loss to keep it in check
    #neg_loss = gama * neg_loss
    ...
    #neg_loss = torch.nan_to_num(neg_loss, nan=1e4, posinf=1e4, neginf=1e4)

    # Print loss components (combined physics loss for display purposes)
    #physics_loss = (res1_loss + res2_loss + res3_loss + res4_loss + resMass1_loss + resMass2_loss) / 6
    #print(f"Data: {data_loss.detach():.4f}, Physics: {physics_loss.detach():.4f}, Reg: {neg_loss.detach():.4f}", end='\r')
    ...
    #return data_loss, res1_loss, res2_loss, res3_loss, res4_loss, resMass1_loss, resMass2_loss, neg_loss
    return data_loss, res1_loss, res2_loss, res3_loss, res4_loss, resMass1_loss, resMass2_loss


def pinn_training_with_algorithm():
    # Device configuration
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    # Create output directory for saving model
    output_dir = 'results/pinn_test'
    ...
    n_epochs = 10000           # Number of epochs (reduced from 10000)
